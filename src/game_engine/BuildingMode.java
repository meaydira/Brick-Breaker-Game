...
public class BuildingMode implements Runnable, GameConstants  {
    ...
    public void initializeMap(){
        ...
        for(int i=0; i<numMineBrick;i++) {
            int xPos=xPosBrick_lowLimit+ random.nextInt(xPosBrick_HighLimit-xPosBrick_lowLimit);
            int yPos=yPosBrick_lowLimit+ random.nextInt(yPosBrick_HighLimit-yPosBrick_lowLimit);
            Brick mineBrick= Brickfactory.getInstance().produce("MineBrick",xPos,yPos,BRICK_WIDTH,BRICK_HEIGHT, Color.white);
            if(!( map==null)) {
        ...
        for(int i=0; i<numHalfMetalBrick;i++) {
            int xPos=xPosBrick_lowLimit+ random.nextInt(xPosBrick_HighLimit-xPosBrick_lowLimit);
            int yPos=yPosBrick_lowLimit+ random.nextInt(yPosBrick_HighLimit-yPosBrick_lowLimit);
            Brick halfMetalBrick= Brickfactory.getInstance().produce("HalfMetalBrick",xPos,yPos,BRICK_WIDTH,BRICK_HEIGHT, Color.white);
            if(!( map==null)) {
    ...
    public void handleClick(int x , int y){
        ...
        if(isProcessed == false){
            switch (currentBrick){
                case 1:
                    Brick simpleBrick= Brickfactory.getInstance().produce("SimpleBrick",x,y,BRICK_WIDTH,BRICK_HEIGHT, Color.white);
                    if(!(Intersect.checkIntersectSimpleBrickwithAllBricks(simpleBrick,map))){
                        map.addBrick(simpleBrick);
                    }
                    break;
                ...
                case 2:
                    Brick halfMetalBrick= Brickfactory.getInstance().produce("HalfMetalBrick",x,y,BRICK_WIDTH,BRICK_HEIGHT, Color.white);
                    if(!(Intersect.checkIntersectSimpleBrickwithAllBricks(halfMetalBrick,map))){
                        map.addBrick(halfMetalBrick);
                    }
                    break;
                case 3:
                    Brick mineBrick= Brickfactory.getInstance().produce("MineBrick",x,y,BRICK_WIDTH,BRICK_HEIGHT, Color.white);
                    if(!(Intersect.checkIntersectSimpleBrickwithAllBricks(mineBrick,map))){
                        map.addBrick(mineBrick);
                    }
                    break;
